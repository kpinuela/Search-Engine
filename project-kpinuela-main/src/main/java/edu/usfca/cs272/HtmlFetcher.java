...
public class HtmlFetcher {
	...
	public static int getStatusCode(Map<String, List<String>> headers) {
		if (headers.containsKey(null)){
			return Integer.parseInt(headers.get(null).toString().split(" ")[1]);
		}
		return -1;
	}
	...
	public static String fetch(URL url, int redirects) {
		...
		try (
			...
			Map<String, List<String>> fetched= HttpsFetcher.fetchURL(url);
				HttpsFetcher.printGetRequest(request, url);
				HttpsFetcher.getHeaderFields(response);
			
			if(getStatusCode(fetched)==200 && isHtml(fetched) ) {
				return String.join("\n", fetched.get("Content"));
				
			}
			if(isRedirect(fetched)&&redirects>0){
				return fetch(fetched.get("Location").get(0), redirects - 1);			
			}
			else {
				return null;
			}
